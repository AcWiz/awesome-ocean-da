...
### 硬件配置
- GPU: NVIDIA V100 或 RTX 2080 Ti（推理）
- GPU数量: 1块（单GPU训练）
- 训练时间: 未明确说明（由于网络规模较小（2层128单元），训练时间预计较短，约数分钟至数十分钟）

### 数据集（Datasets）
1. **局部平流扩散方程（Local Advection-Diffusion Equation）合成数据集**
   - 来源: 通过数值求解随机偏微分方程生成
   - 任务: 不确定性传播与代理模型构建
   - 数据规模: 时间网格点 nt=128，空间网格点 nz=64，样本数 ns=100
   - 是否公开: 否（论文中未提及公开数据集）

### 数据处理
- 随机参数采样： diffusivity 参数遵循指数高斯过程分布
- 网格生成： 在时空域 [0,1]×[0,1] 上生成均匀网格
- 训练样本构建： 将时空网格展平后作为输入，batch size B = nt × nz

### 复现难度
- ★★★☆☆（中等）
- 原因：网络架构简单明确（全连接网络），优化器和超参数已详细说明；但未公开源代码和预训练权重，且合成数据集的生成过程需要复现随机偏微分方程的求解器，增加复现难度。
