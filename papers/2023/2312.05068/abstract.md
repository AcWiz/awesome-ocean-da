...
### 硬件配置
- GPU: NVIDIA A100 或 V100（基于2023年深度学习研究的标准配置）
- GPU数量: 单GPU训练（未明确说明多GPU配置）
- 训练时间: 未明确说明具体训练时长

### 数据集（Datasets）
1. **NEMO全球海洋模型数据**
   - 来源: NEMO (Nucleus for European Modelling of the Ocean) 全球海洋配置
   - 任务: 归一化系数估计
   - 数据规模: 全球海洋网格，约1度水平分辨率
   - 是否公开: 不确定（模型配置可能来自业务化系统）

2. **归一化系数真值数据**
   - 来源: 通过暴力法（brute-force）对2D扩散算子进行精确计算生成
   - 任务: CNN监督学习的标签数据
   - 数据规模: 与全球海洋网格分辨率相匹配
   - 是否公开: 不确定

### 数据处理
- 将原始8个输入场（κ11, κ22, e1v, e2v, e1u, e2u, w, m）简化为3个通道（α1, α2, w），其中α1和α2为缩放后的扩散率场，w为网格单元面积
- 输入数据基于Arakawa C网格的交错布局
- 训练数据由精确计算的归一化系数作为监督标签
- 采用2D扩散算子进行实验，扩散步数M=10

### 复现难度
- ★★★☆☆（中等难度）
- 原因：代码已在GitHub公开（https://github.com/FolkeKS/DL-normalization/tree/core-features），但训练数据和详细的实验参数（如学习率、批大小等）未完全在文中披露。此外，全球海洋模型配置可能需要额外获取NEMO模型的相关资源。
