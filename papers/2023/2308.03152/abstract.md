...
source: arXiv
venue: arXiv
method_tags: [Masked_Autoencoder, Fourier_Based, Backbone_Downstream, Global_Ocean_Modeling,
  Deep_Learning]
application_tags: [Global_Ocean_Modeling, Ocean_Prediction, Mesoscale_Eddies, Ocean_Variables,
  Earth_System]
difficulty: ★★★★★
importance: ★★★★★
...
### 硬件配置
- GPU: NVIDIA A100 或 V100（大型深度学习模型训练的典型选择）
- GPU数量: 8-64块（大规模模型训练通常需要多GPU并行）
- 训练时间: 未明确说明

### 数据集（Datasets）
1. **HYCOM（HYbrid Coordinate Ocean Model）全球再分析数据**
   - 来源: GODAE（全球海洋资料同化实验）官方发布
   - 任务: 全球海洋基本变量（温度、盐度、流速、海表面高度）预测
   - 数据规模: 10年训练数据（2000-2010），1年验证（2011），1年测试（2012）；空间分辨率1/4°，15个深度层（0-500m）
   - 是否公开: 是（公开可用的海洋再分析数据集）
...
### 数据处理
- 海洋变量标准化：使用Z-score或Min-Max归一化处理温度、盐度、流速和海表面高度
- 深度层处理：将15个深度层的物理场通过Patch嵌入结构映射为一维序列token
- 多源数据融合：大气强迫数据（ERA5）和地形拓扑数据（ETOPO）作为边界条件输入
- 随机掩码策略：随机遮挡部分patch以增强模型特征提取能力，防止过拟合
- 网格格式：二维/三维规则网格数据输入

### 复现难度
- ★★★☆☆（中等难度）
- 原因：HYCOM、ERA5和ETOPO数据集均为公开可获取的标准数据集，数据处理方法有明确描述。但论文未明确说明模型代码是否开源，训练超参数（如学习率、批量大小、epoch数）未详细披露，且未提供预训练模型权重。对于大型深度学习模型的完整复现，需要较大的计算资源（多GPU集群）和充分的调参经验。
