...
- GPU数量: 未明确说明（推测为单卡或小规模集群）
- 训练时间: 未明确说明（考虑到5000个模型集合的训练开销，预计需要数天至数周）

### 数据集（Datasets）
1. **太平洋海表温度（SST）数据**
   - 来源: 推测为HadISST或ERSSTv5等国际公开数据集（具体来源需参照原文）
   - 任务: Nino3.4区域SST预测
   - 数据规模: 赤道太平洋区域（约180°E-80°W，20°S-20°N），时间跨度覆盖ENSO事件的多个周期
   - 是否公开: 是

### 数据处理
- 将SST数据进行标准化或归一化处理以适配CNN输入
- 根据三组实验设计裁剪不同的空间区域（西太平洋、东太平洋、全太平洋）
- 将数据划分为训练集、验证集和测试集
- 采用时间序列滑动窗口方法生成训练样本

### 复现难度
- ★★★☆☆（中等难度）
- 原因：SST数据为公开可用数据集，获取难度较低；CNN架构相对标准且文中给出了详细的模型配置；但5000个模型集合的训练计算量较大，且未明确说明代码是否公开，需要较多的计算资源投入。
