...
### 硬件配置
- GPU: NVIDIA A100 或 V100（推断为深度学习实验常用GPU）
- GPU数量: 单GPU
- 训练时间: 未明确说明（考虑到数据规模较小，训练时间预计在数小时级别）

### 数据集（Datasets）
1. **卫星高度计海表面高度数据**
   - 来源: 卫星测高任务（1993-2022）
   - 任务: 海平面趋势预测
   - 数据规模: 1/4度空间分辨率，约30年月度数据；重采样后为2度（180×90）网格
   - 是否公开: 是（卫星高度计数据通常公开可用）

2. **气候模式大集合模拟数据**
   - 来源: CESM1, CESM2, GFDLESM2M, MPIGE, MPI-ESM1-2-HR, MPI-ESM1-2-LR六个气候模式
   - 任务: 提供气候模型海表面高度趋势作为输入特征
   - 数据规模: 1度空间分辨率，1993-2022年及2023-2052年预测期
   - 是否公开: 是（气候模式大集合通常公开可用）

### 数据处理
- 移除季节性信号：对卫星高度计数据进行去季节化处理
- 全局平均移除：对所有数据集移除全局平均以保证一致性
- 空间重采样：将原始数据统一重采样至2度分辨率（180×90网格）以加速计算
- 空间平滑：对卫星高度计数据进行空间平滑以减少小尺度海洋涡旋影响
- 趋势计算：对每个网格点拟合1993-2022年及2023-2052年的线性趋势
- 网格展平：将空间网格展平为样本，每个网格点作为一个独立样本

### 复现难度
- ★★★☆☆（中等难度）
- 原因：数据集来源明确且多为公开可用数据（卫星高度计和气候模式大集合均可获取），方法采用标准FCNN架构较易复现。但谱聚类分区需要专业领域知识，且论文未提供具体代码实现细节，5折交叉验证的超参数选择过程也未完全披露，增加了复现的技术门槛。
