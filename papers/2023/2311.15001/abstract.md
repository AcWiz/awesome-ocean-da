...
1. **ECMWF-IFS数值预报数据**
   - 来源: 欧洲中期天气预报中心 (ECMWF)
   - 任务: 显著波高 (SWH) 0-240小时数值预报
   - 数据规模: 西太平洋区域网格化数据，分辨率未明确说明
   - 是否公开: 否（需通过ECMWF数据接口获取）
...
3. **SWH历史观测/再分析数据**
   - 来源: 卫星高度计或再分析产品（如ERA5）
   - 任务: 作为训练标签或验证参考
   - 数据规模: 与预报数据时空范围一致
   - 是否公开: 是

### 数据处理
- **空间网格化**: 将ECMWF IFS预报数据统一插值至目标网格
- **时间对齐**: 将SWH预报序列与对应时次的10米风场数据对齐
- **归一化处理**: 对SWH和风场数据进行标准化或最大-最小归一化
- **时空位置编码**: 添加时间和空间位置信息以捕捉时空依赖
- **季节性划分**: 按气象季节（3-5月春、6-8月夏、9-11月秋、12-2月冬）分割训练数据
- **时序切片**: 采用滑动窗口方式构建时序样本，用于GRU模型输入
- **数据划分**: 按时间顺序划分为训练集、验证集和测试集

### 复现难度
- ★★★☆☆（中等难度）
- **原因**：1) ECMWF-IFS原始预报数据非完全公开，需申请数据权限；2) 模型架构（Trajectory GRU和像素切换损失函数）需根据论文描述自行实现；3) 论文未明确给出超参数设置和训练细节；4) 时空数据的预处理流程需根据研究区域特点重新调整；5) 若使用公开的再分析数据（如ERA5）替代原始IFS数据，结果可能存在差异
