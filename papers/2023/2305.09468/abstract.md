...
### 硬件配置
- GPU: NVIDIA A100 或 V100（典型深度学习GPU）
- GPU数量: 未明确说明（单GPU训练）
- 训练时间: GraphSAGE模型训练时间最短；整体训练周期为20个epoch

### 数据集（Datasets）
1. **ERA5 SST数据集**
   - 来源: 欧洲中期天气预报中心（ECMWF）ERA5再分析产品
   - 任务: 海表温度（SST）月度预测，预测窗口为1个月
   - 数据规模: 空间分辨率0.25°，处理后为2°×2.8125°网格，共5774个节点；时间范围1950年1月至2022年8月（872个月），训练集760个时间步，测试集112个时间步
   - 是否公开: 部分公开（论文提到图数据已提供下载链接）

### 数据处理
- 使用CDO（Climate Data Operators）将ERA5数据处理为[64, 128, 872]的纬度-经度-时间网格
- 将网格数据展平构建图结构节点属性矩阵
- 使用Pearson相关系数构建邻接矩阵，设置阈值c=0.95（无向图）和c=0.9（有向图）
- 对SST数据进行最小-最大归一化至[-1, 1]范围
- 未使用大气温度变量进行陆地点插值

### 复现难度
- ★★★☆☆（中等难度）
- 原因：数据集部分公开（图数据已提供下载），但完整代码和详细配置文件未明确说明；方法基于标准GNN架构（GCN、GAT、GraphSAGE、RGCN），实现难度较低；图构建方法详细描述，复现性较好；建议联系作者获取完整代码以提高复现成功率
