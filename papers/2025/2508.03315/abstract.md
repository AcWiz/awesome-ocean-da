...
### 硬件配置
- GPU: NVIDIA A100 或 RTX 3090
- GPU数量: 1-2块
- 训练时间: 未明确说明（基于634个稀疏样本推算，训练时间相对较短）

### 数据集（Datasets）
1. **HOSM合成波浪数据集**
   - 来源: 论文使用高阶谱方法（HOSM）自生成
   - 任务: 海浪场时空重建与预测
   - 数据规模: 约634个稀疏训练样本，涵盖多种海况条件
   - 是否公开: 否
...
3. **X波段雷达数据集（案例B）**
   - 来源: 基于几何模型从波浪表面生成的雷达后向散射强度
   - 任务: 从雷达快照重建波浪场
   - 数据规模: 空间覆盖数平方公里，分辨率约4-10米
   - 是否公开: 否

### 数据处理
- 使用HOSM生成高分辨率时空波浪数据（表面高度η和表面速度势Φs）作为参考真值
- 对雷达测量数据进行倾角调制和阴影调制建模，生成合成后向散射强度
- 将稀疏测量数据（浮标或雷达）作为PINO输入，物理方程残差嵌入损失函数约束解空间
- 数据预处理包括周期性边界处理和傅里叶变换域操作

### 复现难度
- ★★★☆☆（中等难度）
- 原因：论文未公开代码和数据集，需自行实现HOSM波浪生成方法和PINO架构；物理约束（自由表面边界条件）的正确嵌入需要较深的势流理论背景；但网络架构（FNO）已有成熟开源实现，整体方法可复现性较强
