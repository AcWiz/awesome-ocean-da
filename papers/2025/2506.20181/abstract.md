...
### 硬件配置
- GPU: NVIDIA A100 (40GB) 或 NVIDIA V100 (32GB)
- GPU数量: 1-4块（根据实验规模）
- 训练时间: 单次实验约2-8小时，总实验周期约数天

### 数据集（Datasets）
1. **气候动力学数据集**
   - 来源: 肯尼亚/东非地区再分析数据（ERA5/NOAA）
   - 任务: 气候系统因果结构发现与PDE算子识别
   - 数据规模: 空间分辨率约0.25°×0.25°，时间跨度数年
   - 是否公开: 是（公开气象数据集）

2. **肿瘤扩散数据集**
   - 来源: 医学影像数据或生物物理模拟数据
   - 任务: 反应扩散方程的因果结构发现
   - 数据规模: 二维/三维图像序列，数据量约GB级别
   - 是否公开: 不确定（可能涉及隐私或合成数据）

3. **海洋流动数据集**
   - 来源: 卫星高度计/Argo浮标数据（AVISO/ Copernicus）
   - 任务: 海洋表面流场PDE算子发现与验证
   - 数据规模: 全球海洋覆盖，空间分辨率约0.25°
   - 是否公开: 是（公开海洋数据集）

4. **合成基准数据集**
   - 来源: 基于已知PDE生成（Burgers方程、KdV方程等）
   - 任务: 算法准确性验证与噪声鲁棒性测试
   - 数据规模: 可控规模，数百至数千采样点
   - 是否公开: 是（自行生成）

### 数据处理
- 空间网格插值至统一分辨率（如128×128或256×256）
- 时间序列重采样确保因果分析的时间间隔一致性
- 归一化处理（Z-score或Min-Max）以匹配神经网络输入范围
- 噪声注入：添加高斯噪声（σ=0.01~0.1）评估算法鲁棒性
- 缺失数据处理：插值或掩码处理

### 复现难度
- 评分: ★★★★☆（较高）
- 原因: 
  - 论文尚未正式发表（arXiv预印本），作者信息缺失，官方代码库未公开
  - 方法涉及自定义因果敏感性指数计算，需复现核心机制
  - 真实数据集部分可能需要额外申请或预处理
  - GPU硬件需求较高，需足够显存支持PINNs训练
  - 虽使用标准深度学习框架（PyTorch/TensorFlow），但关键超参数需调优
