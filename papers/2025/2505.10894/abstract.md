...
- GPU数量: 1
- 训练时间: 未明确说明（50 epochs，batch size 32）

### 数据集（Datasets）
1. **南海（SCS）与黑潮（KUR）海洋锋面数据集**
   - 来源: NOAA Coral Reef Watch 5km SST产品（v3.1）及Copernicus海洋服务全球海洋物理再分析产品
   - 任务: 海洋锋面预测（单步及多步预测）
   - 数据规模: 总样本10220个，空间分辨率统一为9km（300×300），时间范围1993-2020年（10227条日记录）
   - 是否公开: 部分公开（SST与 velocity数据可从NOAA和Copernicus下载）

### 数据处理
- 分辨率统一至9km，空间维度300×300
- 输入为7×3×300×300（7天序列，3个变量：锋面区、u速度、v速度）
- 目标为1×3×300×300
- 训练集与测试集比例8:2（8176:2044）
- 采用微正则多尺度形式主义（MMF）进行锋面检测

### 复现难度
- ★★★☆☆（中等难度）：论文提供了详细的网络架构、超参数设置及公开数据源，但未明确说明代码是否开源，且需要整合多源海洋数据并实现复杂的CNN-Transformer-PINN混合架构
