...
### 硬件配置
- GPU: NVIDIA A100
- GPU数量: 1-4块（未明确说明具体数量，典型深度学习fine-tuning任务配置）
- 训练时间: 未明确说明

### 数据集（Datasets）
1. **GLORYS12V1**
   - 来源: 哥白尼海洋服务（Copernicus Marine Service）全球海洋物理再分析数据
   - 任务: 海表温度（SST）预测及海洋三维温度场重建
   - 数据规模: 水平分辨率1/12°，50个垂向层次，日数据；覆盖加那利涌升系统区域
   - 是否公开: 否（需注册订阅Copernicus服务）

### 数据处理
- 数据归一化/标准化处理以适配模型输入格式
- 从全球数据中裁剪加那利涌升系统子区域（约北非西海岸附近）
- 采用纬度加权误差度量（latitude-weighted error metrics）以优化高纬度区域学习效果
- 两阶段微调策略：第一阶段全参数微调，第二阶段针对性参数微调

### 复现难度
- ★★★☆☆（中等难度）
- 原因：虽然模型架构（Aurora）和方法论已描述，但GPU数量、训练时间等硬件细节未明确；数据集GLORYS12V1需付费订阅，无法直接获取；论文未提供开源代码链接，复现所需的关键超参数设置需进一步推敲。
