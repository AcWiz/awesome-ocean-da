...
- GPU数量: 1-2块（单GPU训练或小规模多GPU配置）
- 训练时间: 未明确说明

### 数据集（Datasets）
1. **北极海冰遥感数据集**
   - 来源: 美国国家冰雪数据中心（NSIDC）及欧洲气象卫星组织（EUMETSAT）OSI-SAF
   - 任务: 海冰速度（SIV）和海冰密集度（SIC）的每日预测
   - 数据规模: 北极海域全覆盖，时间跨度覆盖融化季和初冻季典型时段
   - 是否公开: 部分公开（NSIDC提供公开数据访问）
...
### 数据处理
- 将遥感海冰观测数据重采样至统一空间分辨率（百公里级网格）
- 采用min-max归一化或z-score标准化处理输入特征
- 按时间顺序划分训练集、验证集和测试集
- 处理缺失值和质量控制：剔除异常值和云污染像元
- 基于物理方程设计物理损失函数，包括质量守恒和热力学约束项

### 复现难度
- ★★★☆☆（中等难度）
- 原因：实验配置较为标准，但物理损失函数的具体权重系数和训练超参数未在文中详细披露；HIS-Unet架构需参考作者先前工作；数据获取渠道公开，但完整预处理流程需额外摸索；代码未公开提供
