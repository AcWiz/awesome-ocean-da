...
### 数据集（Datasets）
1. **东北太平洋沿海海平面数据集**
   - 来源: NOAA验潮站数据及海表高度再分析产品
   - 任务: 沿海海表高度亚季节至季节概率分布预测
   - 数据规模: 东北太平洋海岸沿线的网格点数据，包含多年周尺度海表高度时间序列
   - 是否公开: 不确定（arXiv论文通常不附带完整数据集）

2. **聚类标签数据**
   - 来源: 基于K-means或层次聚类算法生成的区域划分
   - 任务: 作为Whole Coast和Cluster任务的预处理输入
   - 数据规模: 预先划分的若干聚类区域
   - 是否公开: 否

### 数据处理
- 海表高度异常计算：去除季节循环和气候态均值
- 时间尺度聚合：将原始数据聚合至周尺度（尤其是Week 3预测目标）
- 数据归一化：采用标准化处理以适配神经网络输入
- 网格点预处理：沿东北太平洋海岸线选取代表性网格点
- 聚类分析：使用聚类算法将网格点分组以识别协方差结构

### 复现难度
- ★★★☆☆（中等难度）
- 原因：该研究使用CNN进行沿海海平面预测，方法框架清晰可复现，但存在以下限制：①论文未提供代码开源链接；②海洋观测数据集的获取可能受限，需联系数据提供机构；③聚类区域划分策略需要进一步调试以达到最优效果。整体而言，具备深度学习基础和海洋学背景的研究者可尝试复现。
