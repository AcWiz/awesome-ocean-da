...
### 硬件配置
- GPU: NVIDIA A100 (基于深度学习海洋预报模型的典型硬件需求推断)
- GPU数量: 8 GPU (论文明确提到在8-GPU集群上训练SwinTransformer变体)
- 训练时间: 单个SwinTransformer变体约30小时，完整实验因包含多模型及超参数调优，总训练时间显著增加

### 数据集（Datasets）
1. **GLORYS12 (训练数据)**
   - 来源: 哥白尼海洋环境监测服务 (CMEMS)
   - 任务: 提供海洋温度、盐度、海流等变量用于模型训练
   - 数据规模: 约13TB，涵盖1993-2021年共28年数据，包含50个垂向层次
   - 是否公开: 是

2. **ERA5 (训练数据)**
   - 来源: 欧洲中期天气预报中心 (ECMWF)
   - 任务: 提供海表风场数据作为海洋环流的外力驱动
   - 数据规模: 约170GB
   - 是否公开: 是
...
4. **EN4 (评估数据)**
   - 来源: 英国气象局哈德莱中心
   - 任务: 评估海洋温度和盐度预报性能
   - 数据规模: 涵盖1900-2025年，包含约3000个Argo剖面浮标质控数据
   - 是否公开: 是

5. **GDP (评估数据)**
   - 来源: 全球漂流浮标计划
   - 任务: 评估海表温度和海流预报性能
   - 数据规模: 6小时间隔插值的SST和海流数据
   - 是否公开: 是

6. **CMEMS L3 (评估数据)**
   - 来源: 哥白尼海洋环境监测服务
   - 任务: 评估海面高度异常预报性能
   - 数据规模: 1993年至今的高分辨率卫星观测
   - 是否公开: 是

### 数据处理
- **空间标准化**: 将多源数据统一重采样至1.40625°水平分辨率
- **垂向插值**: 提取23个标准垂向层次（0.49m至643.57m）
- **时间同步**: 统一至每日或每6小时时间分辨率
- **数据融合**: 将GLORYS12、ERA5、OSTIA三源数据按时间维度拼接为统一训练集
- **观测匹配**: 建立标准化评估流程，将预报数据与离散观测点（浮标、卫星）进行空间时间对齐

### 复现难度
- ★★☆☆☆ (较易复现)
- **原因**: 论文已在GitHub开源数据集和代码（https://github.com/Ocean-Intelligent-Forecasting/OceanForecastBench），所有训练和评估数据均来自公开可用的权威海洋数据集（GLORYS12、ERA5、OSTIA、EN4、GDP、CMEMS），基线模型均为公开模型。唯一挑战在于8-GPU集群的计算资源需求较高，但实验配置（模型选择、超参数）已详细披露。
