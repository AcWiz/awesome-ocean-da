...
- GPU数量: 1-4 块（典型学术研究配置）
- 训练时间: 未明确说明

### 数据集（Datasets）
1. **Copernicus Marine Service L4 SST 再分析产品**
   - 来源: 哥白尼海洋服务（Copernicus Marine Service）
   - 任务: 海表温度（SST）预测与中尺度海洋过程建模
   - 数据规模: 1982-2020年共39年每日数据，覆盖加那利上升流系统区域
   - 是否公开: 是（需注册获取）

### 数据处理
- 海表温度场标准化处理（归一化至[0,1]或标准差范围）
- 空间掩膜损失函数处理陆地不连续性问题
- 时间序列构建：采用滑动窗口生成多时间步输入-输出对
- 数据增强：卫星观测数据填补与质量控制
- 区域裁剪：聚焦于IBI区域（伊比利亚、比斯开、爱尔兰）加那利流上升流子区域

### 复现难度
- ★★★☆☆（中等难度）
- 原因：数据集（Copernicus SST）可通过官方渠道公开获取，但模型架构细节（GraphCast改编版本）未在文中完全公开，训练超参数和具体实现细节缺失；无开源代码仓库提供；传统数值模式（GLORYS、PSY4V3R1）可通过哥白尼系统访问，但模型间对比的基准配置未详细说明。
