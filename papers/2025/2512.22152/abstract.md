...
- GPU数量: 1-4块 (多GPU并行训练以加速收敛)
- 训练时间: 约数天至一周 (基于1年训练数据量及序列到序列任务复杂度推断)

### 数据集（Datasets）
1. **GLORYS12 (GLORYS12V1)**
   - 来源: Mercator Ocean International海洋再分析产品
   - 任务: 作为训练数据的真值场，支持OSSE实验
   - 数据规模: 全球1/12°分辨率，日均数据，1年训练数据
   - 是否公开: 是

2. **GLO12 (Mercator Ocean预报产品)**
   - 来源: Mercator Ocean International业务化数值预报系统
   - 任务: 作为基线对比模型
   - 数据规模: 全球1/12°分辨率，7-10天预报时效
   - 是否公开: 是（商业产品，但研究可用）

3. **卫星高度计观测数据 (nadir altimeters)**
   - 来源: 合成星下点采样模式，模拟Jason-3/Sentinel-6等任务
   - 任务: 作为模型输入的稀疏观测
   - 数据规模: 14天历史观测序列
   - 是否公开: 不确定（实际卫星数据需申请）

### 数据处理
- 海表高度异常(SLA)提取与标准化预处理
- 14天历史稀疏观测窗口构建
- 合成星下点采样模式应用于真值场以模拟稀疏观测
- 空间网格化处理：1/12°分辨率经纬度网格
- 归一化处理：基于全局统计量或区域标准化

### 复现难度
- ★★★☆☆ (中等难度)
- 原因：① 论文提及OceanBench基准测试倡议及GitHub代码仓库，具有一定开源意向；② GLORYS12和GLO12数据集为海洋学界常用产品，可公开获取；③ 但具体代码实现细节未在文中完全披露，需参考外部代码库；④ 合成观测模式的具体参数未明确给出
