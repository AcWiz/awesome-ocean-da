...
### 硬件配置
- GPU: NVIDIA A100 (基于深度学习海洋预报模型的典型硬件配置)
- GPU数量: 1块 (推理过程使用单GPU)
- 训练时间: 未明确说明

### 数据集（Datasets）
1. **Mediterranean Reanalysis Data**
   - 来源: CMCC Mediterranean Reanalysis (MED-MFC)
   - 任务: 模型预训练
   - 数据规模: 35年 (1987-2021)，日均数据
   - 是否公开: 不确定

2. **Mediterranean Operational Analysis**
   - 来源: CMEMS Mediterranean Forecasting System (MedFS)
   - 任务: 模型微调
   - 数据规模: 2年 (2022-2023)，日均数据
   - 是否公开: 否

3. **ERA5大气强迫数据**
   - 来源: ECMWF
   - 任务: 表面大气强迫输入 (风应力、2米温度、海平面气压)
   - 数据规模: 1987-2023
   - 是否公开: 是

4. **卫星观测数据**
   - 来源: 多传感器合并卫星产品 (SST L3 1/16°分辨率) 和沿轨迹高度计数据 (SLA)
   - 任务: 模型验证
   - 数据规模: 2024年7月至12月测试期
   - 是否公开: 是

### 数据处理
- 水平分辨率统一至1/24°(约4公里)，与业务化MedFS系统一致
- 模型输出18个深度层，包含73个预报变量 (深度分辨的纬向/经向流、盐度、温度及海表高度)
- 采用层次化图网格表示，将输入海态和大气强迫编码至粗分辨率网格，经GNN层处理后再解码回原始高分辨率网格
- 模型学习日间隔内的变化量(倾向)，通过叠加当前状态得到预报
- 边界条件处理：在直布罗陀海峡和达达尼尔海峡区域施加真实海态(训练期)或MedFS数据(评估期)

### 复现难度
- ★★★☆☆ (中等难度)
- 原因：论文未公开代码仓库，数据集来源涉及业务化系统(MED-MFC/MedFS)，需特殊权限获取；大气强迫(ERA5/ENS/AIFS)和卫星观测数据可公开获取，但完整复现需获取地中海再分析数据，且模型架构细节需参考补充材料推断
