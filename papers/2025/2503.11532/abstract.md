...
- GPU数量: 1-4块（单卡训练或分布式训练）
- 训练时间: 约数小时至数天（取决于patch数量和模型复杂度）

### 数据集（Datasets）
1. **CMEMS地中海多传感器海洋光学产品**
   - 来源: 哥白尼海洋环境监测服务（CMEMS）
   - 任务: 海洋光学参数gap filling / 卫星数据重建
   - 数据规模: 地中海区域，空间分辨率300m-1km，时间覆盖数月至数年
   - 是否公开: 是（需注册下载）

2. **多源卫星观测数据**
   - 来源: MODIS、VIIRS、SeaWiFS、OLCI传感器
   - 任务: 多传感器数据融合与缺失填补
   - 数据规模: 每日至每16-27天重访，覆盖可见光至近红外波段
   - 是否公开: 是（通过CMEMS平台获取）

### 数据处理
- 大气校正：将Level-1数据校正为Level-2海表产品
- 质量控制：标识并剔除异常值和陆地区域
- 多传感器融合：整合不同传感器的观测数据
- 网格化处理：统一空间分辨率至标准网格
- Patch提取：从原始gappy数据中随机裁剪固定大小patch
- 随机掩膜：对patch应用随机缺失模式模拟不同gap分布
- 数据归一化：基于研究区统计特征进行标准化处理

### 复现难度
- ★★★☆☆（中等）
- 原因：方法框架基于开源4DVarNet架构，但CMEMS数据需注册获取且存在使用限制；patch重采样策略和4DVarNet具体超参数未完全公开；研究为2025年新发表，代码和预训练模型尚未发布。
