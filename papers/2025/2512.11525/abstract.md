...
### 数据集（Datasets）
1. **GLORYS12 + ERA5 融合数据集**
   - 来源: GLORYS12（海洋状态变量），ERA5（大气强迫场）
   - 任务: 全球海洋环流模拟预测
   - 数据规模: 1993-2020年，时间分辨率24小时，空间分辨率1.5°，包含5个海洋变量（温度、盐度、U/V速度场23层，SSH单层）及4个大气变量
   - 是否公开: 不确定

### 数据处理
- 数据标准化：在归一化空间计算损失函数
- 按时序分割：训练集（1993-2017）、验证集（2018-2019）、测试集（2020）
- 应用静态陆海掩膜（LSM）定义海洋区域
- 球面网格处理：经向边界周期性填充，考虑纬度依赖度量因子

### 复现难度
- ★★★☆☆（中等）
- 原因：论文提及代码将公开，但未提供具体链接；数据集（GLORYS12、ERA5）通常需申请获取权限；方法涉及自定义可微分物理求解器，复现具有一定复杂度
