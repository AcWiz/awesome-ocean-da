...
### 硬件配置
- GPU: 不适用（本研究采用线性回归与奇异值分解方法，无需GPU加速）
- GPU数量: 不适用
- 训练时间: 未明确说明（由于采用线性统计方法，计算量较小，在标准工作站上数分钟至数小时即可完成）
...
2. **NCEP/NCAR Reanalysis**
   - 来源: NOAA国家环境预测中心（NCEP）与美国国家大气研究中心（NCAR）
   - 任务: 提供地表风场（U、V分量）及海平面气压（P）数据
   - 数据规模: 1948-2025年月度数据，与ERSSTv5相同的时间空间范围
   - 是否公开: 是

### 数据处理
- 对原始数据（未做距平化处理）直接进行SVD分解，提取四个主导季节模态
- 对每个模态的时间序列计算距平值，得到16个前兆因子（4个SST、4个U风、4个V风、4个气压）
- 采用3个月滑动平均对区域指数进行平滑处理，以抑制次季节噪声
- 数据划分：训练期（1948-2000年，共53年）、验证期（2001-2023年）、独立预报期（2024-2025年）

### 复现难度
- ★★☆☆☆（较低）
- 原因：研究所用数据均为公开可获取的权威再分析数据集，方法为标准的SVD与多元线性回归，技术成熟且实现简单。然而论文未提供代码实现细节，仅描述了数学框架和公式，对于不熟悉该领域的研究者可能存在一定复现门槛，但整体难度不大。
