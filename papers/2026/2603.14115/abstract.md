...
### 硬件配置
- GPU: NVIDIA A100 (或类似高性能GPU，如V100/H100)
- GPU数量: 单GPU训练（未明确说明多GPU并行）
- 训练时间: 未明确说明具体训练时长

### 数据集（Datasets）
1. **双层准地转流数据集 (Two-layer Quasi-Geostrophic Flow)**
   - 来源: 数值模拟生成（基于准地转动力学方程）
   - 任务: 拉格朗日数据同化与预报
   - 数据规模: 未明确说明具体规模；包含表面示踪剂轨迹观测及对应的欧拉流场真值
   - 是否公开: 否（基于数值模拟的合成数据）

### 数据处理
- 采用双层准地转流模型生成高分辨率流场数据
- 在流场表面随机部署示踪剂粒子，记录其时间演化轨迹
- 对示踪剂轨迹进行降采样以模拟稀疏观测
- 添加高斯过程噪声以模拟观测误差和模型不确定性
- 使用傅里叶插值或积分方法计算示踪剂位移

### 复现难度
- ★★★★☆（较难复现）
- 原因：论文未公开代码实现细节；双层准地转流数据集需自行根据物理方程构建；LaCGKN架构包含多个自定义模块（示踪剂同质化、傅里叶位置编码、低秩参数化），需要较高的深度学习与数据同化领域专业知识；缺乏公开的基准数据集对比
