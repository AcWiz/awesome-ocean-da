...
### 数据集（Datasets）
1. **SST重建数据集**
   - 来源: 历史海表温度重建数据（如HadISST或类似长期SST数据集）
   - 任务: 提供大集合模拟的边界强迫条件
   - 数据规模: 1900-2010年共110年全球SST数据
   - 是否公开: 是

2. **再分析数据集**
   - 来源: ERA-Interim/JRA-55等大气再分析资料
   - 任务: 验证阻塞频率的观测基准与模型性能对比
   - 数据规模: 20世纪至2010年代的大气环流历史重建
   - 是否公开: 是

3. **CMIP6模式输出**
   - 来源: CMIP6多模式历史模拟结果
   - 任务: 作为基准与传统物理气候模式进行性能对比
   - 数据规模: 多模式多成员集合
   - 是否公开: 是

### 数据处理
- SST边界条件预处理：将历史SST重建数据插值至模型所需网格分辨率
- 阻塞识别算法：采用标准阻塞指数（如2.5天经向梯度指数或位势高度异常指数）识别阻塞事件
- 集合平均处理：对多个集合成员进行平均以抑制内部大气混沌变率，提取SST强迫信号
- 遥相关分析：计算阻塞频率与SST/ENSO模式之间的统计相关性

### 复现难度
- ★★★☆☆（中等难度）
- 原因：再分析数据和CMIP6数据均公开可获取，但深度学习GCM模型（DLESYM和Neural-GCM）本身的可访问性未明确说明；大集合模拟的计算资源需求较高；阻塞识别算法的具体参数设置需参考原文补充材料
