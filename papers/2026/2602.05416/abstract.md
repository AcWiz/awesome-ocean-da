...
### 数据集（Datasets）
1. **沿海水动力学测试案例库**
   - 来源: 物理水动力模型（如ADCIRC、Delft3D、MIKE 21）模拟结果与现场观测数据
   - 任务: 海表面高度和深度平均流速的长期预测
   - 数据规模: 跨越三个不同动力 regime，预测时间范围长达一年，时间分辨率30分钟
   - 是否公开: 不确定

### 数据处理
- 对高分辨率物理模型输出进行降采样和对齐处理
- 采用Proper Orthogonal Decomposition (POD)进行空间降维，将超过40,000个网格点压缩至低维潜在空间
- 对气象强迫场（风场、气压）和边界条件进行标准化预处理
- 时间序列数据按固定时间窗口分割用于训练和验证
- 海表面高度和流速分量分别进行归一化处理

### 复现难度
- ★★★☆☆（中等）— 论文未公开代码仓库和完整数据集信息，但方法描述较为详细，包括Koopman自编码器架构、POD基代理模型和时域展开技术的关键参数。若需复现需自行获取沿海海洋观测数据和建立相应的物理模型基准数据。
