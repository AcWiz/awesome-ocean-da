...
title: Probabilistic reconstruction of global sea surface temperature using generative
  diffusion models
arXiv: '2603.16272'
authors: [Haijie Li, Ya Wang, Kai Yang, Gang Huang, Xiangao Xia, Ziming Chen, Weichen
    Tao, Chenglin Lu, Lin Chen, Miao Zhang, Kaiming Hu, Hainan Gong, Disong Fu, Lin
    Wang]
year: 2026
source: arXiv
venue: arXiv
method_tags: [Diffusion_Model, Generative_Model, Uncertainty_Quantification, Data_Assimilation,
  Probabilistic_Reconstruction]
application_tags: [Sea_Surface_Temperature, Climate_Prediction, Ocean_State_Estimation,
  El_Niño_Forecasting]
difficulty: ★★★★☆
importance: ★★★★★
read_status: skim
...
- GPU数量: 1-4块（根据实验规模）
- 训练时间: 约数天至一周（完整训练），条件重建单次采样约数分钟

### 数据集（Datasets）
1. **SST再分析数据**
   - 来源: ERA5/Ostia再分析产品
   - 任务: 训练扩散模型先验，学习SST时空变化模式
   - 数据规模: 1982年至今的全球SST日均值，空间分辨率约0.05°-0.25°
   - 是否公开: 是

2. **风云-3D卫星SST产品**
   - 来源: 中国国家卫星气象中心
   - 任务: 提供卫星观测约束，用于大尺度结构重建
   - 数据规模: 微波和红外双模态SST日产品
   - 是否公开: 部分公开

3. **现场观测数据**
   - 来源: Argo浮标剖面、TAO/TRITON浮标阵列、船舶观测（COADS）
   - 任务: 局部特征细化，极端值修正
   - 数据规模: 全球分布的稀疏观测点
   - 是否公开: 是（通过GTS/Argo项目）
...
### 数据处理
- **空间网格化**: 将多源数据插值至0.25°或更高分辨率的规则网格
- **质量控制**: 基于观测误差估计进行数据筛选，去除明显异常值
- **时序对齐**: 所有数据统一至10天复合窗口（ dekadal）
- **归一化处理**: 基于气候态均值和标准差进行标准化
- **缺失处理**: 利用长期气候均值填充无观测区域

### 复现难度
- ★★★☆☆（中等）
- 理由：方法基于扩散模型的生成式框架，核心算法可参考开源Diffusion实现；但卫星数据源（FengYun-3D）获取存在一定门槛，再分析训练数据的预处理流程未完全公开；条件引导机制的具体实现细节可能影响复现效果；无官方代码仓库的情况下需自行实现引导采样策略
