...
### 数据集（Datasets）
1. **墨西哥湾低氧再分析数据集**
   - 来源: 耦合水动力-生物地球化学模型（Warner et al., 2010）
   - 任务: 每日低氧二分类预测
   - 数据规模: 训练集2009-2020年（约4,383天每日数据），测试集2020-2024年（约1,461天每日数据）
   - 是否公开: 不确定

### 数据处理
- 时间周期编码：对日、季节等周期性特征进行正弦/余弦编码
- 特征归一化：Z-score标准化处理
- 序列窗口构建：固定长度滑动窗口生成输入序列
- 类别不平衡处理：SMOTE过采样或类别加权交叉熵损失函数
- 缺失值处理：线性插值或前向填充

### 复现难度
- ★★★☆☆（中等）
- 代码已在GitHub公开，降低了方法层面的复现难度；但原始数据集未公开获取，仅提供模型权重；硬件配置和具体训练超参数未明确说明；依赖特定的海湾环境再分析模型数据，限制了完全独立复现的可能性。
