...
### 硬件配置
- GPU: NVIDIA A100 或 V100（典型高性能计算配置）
- GPU数量: 未明确说明（可能为单卡或小规模集群）
- 训练时间: 未明确说明（基于MCMC链数量和状态维度的迭代耗时）

### 数据集（Datasets）
1. **SWOT卫星海面高程数据**
   - 来源: NASA (Surface Water and Ocean Topography mission)
   - 任务: 海洋状态估计与数据同化
   - 数据规模: 高维观测数据，覆盖全球海洋表面
   - 是否公开: 是

2. **NOAA海洋漂流器数据**
   - 来源: NOAA (National Oceanic and Atmospheric Administration)
   - 任务: 海洋流场重构与非高斯噪声建模
   - 数据规模: 多站点漂流器轨迹数据
   - 是否公开: 是
...
3. **浅水方程模型数据**
   - 来源: 数值模拟生成
   - 任务: 非线性多层浅水方程状态空间模型验证
   - 数据规模: 状态维度 d~10^4-10^5
   - 是否公开: 不确定（可基于代码复现）
...
### 数据处理
- 对SWOT卫星数据进行网格化预处理与缺失值填补
- 对海洋漂流器数据进行轨迹插值与时间同步
- 应用Gaspari-Cohn taper函数实现观测权重的空间衰减
- 对非高斯噪声（如Student-t分布）采用适当的概率密度评估

### 复现难度
- ★★☆☆☆（较易复现）
- 原因：作者提供了开源代码仓库（https://github.com/ruzayqat/LSMCMC），且实验数据来源（SWOT、NOAA漂流器）均为公开可获取的真实数据集。但复现可能需要配置特定的MCMC采样环境与高性能计算资源。
