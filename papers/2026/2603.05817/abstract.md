...
source: arXiv
venue: arXiv
method_tags: [Sequential_MCMC, Localization, Data_Assimilation, Markov_Chain_Monte_Carlo,
  Ensemble_Methods]
application_tags: [Geophysical_Models, Ocean_Data_Assimilation, Shallow_Water_Equations,
  SWOT, Ocean_Drifters]
difficulty: ★★★★☆
importance: ★★★★☆
read_status: skim
...
### 硬件配置
- GPU: NVIDIA V100 或 A100（通用于高性能计算场景）
- GPU数量: 未明确说明（通常为单卡或小型GPU集群，取决于实验规模）
- 训练时间: 未明确说明（该方法为滤波方法，非传统训练过程，涉及MCMC采样时间）

### 数据集（Datasets）
1. **SWOT卫星海面高程数据**
   - 来源: NASA（Surface Water and Ocean Topography mission）
   - 任务: 海洋表面高度观测的数据同化
   - 数据规模: 高维状态空间（d~10^4-10^5），包含真实卫星观测数据
   - 是否公开: 是
...
3. **合成浅水方程模型**
   - 来源: 数值模拟生成
   - 任务: 非线性多层浅水方程的滤波性能验证
   - 数据规模: 状态维数10^4-10^5，含线性和非线性观测算子
   - 是否公开: 是（通过数值实验复现）

### 数据处理
- SWOT数据：卫星海面高程数据的预处理，包括缺失数据插值、观测噪声建模（高斯与Student-t分布）
- 漂流浮标数据：稀疏观测的空间插值与对齐，处理不规则空间分布
- 观测噪声：分别采用高斯噪声和柯西分布（ν=1）噪声进行鲁棒性测试
- Gaspari-Cohn taper函数：用于观测影响的局部化衰减

### 复现难度
- ★★★☆☆（中等难度）
- 原因：代码已开源（GitHub: ruzayqat/LSMCMC），主要数据集（SWOT、NOAA）为公开数据，但具体实验配置、参数选择及合成模型的详细设置需通过代码和补充材料进一步确认，MCMC采样收敛性验证可能需要较多计算资源
