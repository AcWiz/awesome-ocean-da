...
- 对原始状态数据进行归一化/标准化处理
- 按时间步分割训练集与测试集
- 对测量噪声进行有界假设（符合论文的弱假设条件）
- 可观测函数通过深度神经网络参数化学习

### 复现难度
- ★★★☆☆（中等）
- 原因：论文采用标准深度学习框架和常见动力学基准测试，实现难度适中。但论文未明确提供代码开源地址及详细超参数配置，且实验评估在仿真环境中进行，缺乏真实物理系统数据的具体描述，复现时需根据经验补充部分实现细节。
