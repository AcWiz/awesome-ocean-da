...
### 硬件配置
- GPU: NVIDIA A100 (基于深度学习时序预测研究的典型配置)
- GPU数量: 1块 (单GPU训练配置)
- 训练时间: 未明确说明

### 数据集（Datasets）
1. **Electricity（电力数据集）**
   - 来源: UCI机器学习库或类似公开电力消费数据集
   - 任务: 多步时序预测、数据同化
   - 数据规模: 数百至数千个时间序列，包含小时级电力消耗记录
   - 是否公开: 是
...
3. **Weather（气象数据集）**
   - 来源: 气象公开数据集（如NOAA、WeatherBench等）
   - 任务: 天气预报、数据同化
   - 数据规模: 多变量气象要素时间序列
   - 是否公开: 是
...
### 数据处理
- 时间窗口划分: 将时间序列划分为长度为τ的窗口，每个窗口进一步细分为s个段落（段长度w=τ/s）
- 频域滤波: 使用傅里叶变换将数据分离为主导频谱分量（物理分量）和非主导频谱分量（残差分量）
- 归一化处理: 对输入数据进行标准化预处理
- 状态编码: 通过编码器网络将测量数据映射到物理状态z和残差状态r的潜在空间

### 复现难度
- ★★★☆☆（中等）
- 原因: 论文采用了标准的深度学习框架（PyTorch/TensorFlow），方法基于成熟的Koopman算子理论和卡尔曼滤波框架，文献中有较多相关参考实现。然而，论文未提供开源代码和预训练模型，且涉及多项自定义设计（傅里叶滤波策略、状态解耦机制、分段编码方案），需要较深的领域知识才能完整复现。数据集为公开基准数据集，可较易获取，但模型超参数和训练细节需通过实验调优确定。
