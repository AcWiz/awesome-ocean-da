...
### 硬件配置
- GPU: NVIDIA V100 或 A100（通用深度学习GPU型号）
- GPU数量: 1-2块
- 训练时间: 未明确说明
...
1. **Lorenz系统**
   - 来源: 标准混沌系统基准测试
   - 任务: 非线性动力学状态估计与预测
   - 数据规模: 时间序列采样点约10,000-50,000步
   - 是否公开: 是

2. **Van der Pol振子**
   - 来源: 非线性振动理论标准模型（论文中提及）
   - 任务: 非线性系统识别与估计
   - 数据规模: 模拟生成的时间序列数据
   - 是否公开: 是
...
3. **2D圆柱绕流数据集**
   - 来源: 计算流体动力学模拟
   - 任务: 流体力学场景下的状态估计
   - 数据规模: Reynolds数相关参数化数据
   - 是否公开: 不确定

4. **神经动力学数据**
   - 来源: 尖峰神经元模型（如FitzHugh-Nagumo模型）
   - 任务: 神经动力系统状态估计
   - 数据规模: 时间序列采样数据
   - 是否公开: 不确定
...
### 数据处理
- 将连续时间系统离散化处理为时间序列数据
- 对状态变量进行归一化预处理以提高EDMD和DDPG的训练稳定性
- 利用延迟嵌入方法构建高维观测矩阵作为Koopman算子的输入特征
- 对Koopman算子近似矩阵执行SVD降阶处理

### 复现难度
- ★★★☆☆（中等难度）
- 原因：EDMD和DDPG算法均有成熟的公开实现框架可参考，Koopman算子理论方法清晰明确；但论文未提供具体源代码，且涉及超参数选择（如字典函数基函数数量、SVD截断阶数、DDPG网络结构等）对结果影响较大；此外，论文未明确说明具体实验数据集来源和规模，需根据应用场景自行构建测试用例。
