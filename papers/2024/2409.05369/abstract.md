...
### 硬件配置
- GPU: NVIDIA A100 或 V100（基于深度学习气象预测模型通常采用的高端GPU）
- GPU数量: 未明确说明（推测为单卡或少量GPU训练，因模型规模适中）
- 训练时间: 未明确说明
...
1. **ERA5再分析数据**
   - 来源: 欧洲中期天气预报中心（ECMWF）
   - 任务: 海表温度（SST）预测与验证
   - 数据规模: 1959-2009年训练集（51年），2009-2015年验证集，2016-2022年测试集；包含8个关键海洋和大气变量；空间分辨率0.25°×0.25°
   - 是否公开: 是（ERA5公开可用）
...
3. **ECMWF S2S模式数据**
   - 来源: ECMWF S2S模式集合预报
   - 任务: 物理数值模型对比基准
   - 数据规模: 2015-2020年重叠验证期
   - 是否公开: 部分公开（需申请获取完整S2S数据集）

4. **NMME多模式集合数据**
   - 来源: 北美多模式集合（NOAA主导）
   - 任务: 多模式对比评估
   - 数据规模: 包含22个NWP模式的SST预报
   - 是否公开: 是

### 数据处理
- 数据标准化：对8个海洋和大气变量进行标准化处理，消除量纲差异
- 空间插值：将模型输出统一插值至1°×1°分辨率以便与物理模式对比
- 时间采样：按月尺度采样，构建3D输入（经度×纬度×时间/变量）
- 训练策略：采用滑动窗口方法构建样本，输入历史数据，输出未来1-24个月SST预测
- 评估指标：相关系数（R）和均方根误差（RMSE）

### 复现难度
- ★★★☆☆（中等难度）
- 原因：虽然ERA5、HadISST等数据集公开可用，但模型架构细节（Swin-Transformer的具体配置、3D卷积层参数、注意力机制参数等）未在摘要和结果部分详细披露，且官方代码和预训练模型未提供。此外，物理对比模型（ECMWF S2S、NMME）需要额外申请或订阅，完整复现需要较强的深度学习气象预测经验。
