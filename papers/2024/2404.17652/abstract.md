...
- GPU数量: 未明确说明（推理阶段通常使用单卡或少量GPU）
- 训练时间: 未明确说明（本研究使用预训练模型进行推理评估，未涉及模型训练）

### 数据集（Datasets）
1. **ERA5再分析数据**
   - 来源: ECMWF Copernicus Climate Data Store
   - 任务: 作为地面真值（ground truth）进行模型验证
   - 数据规模: 覆盖三次极端天气事件的时间窗口（2021年6-7月、2023年4-5月、2021年2月），空间分辨率为0.25°×0.25°
   - 是否公开: 是

2. **GraphCast模型输出**
   - 来源: Google DeepMind（预训练模型）
   - 任务: 天气预报预测
   - 数据规模: 0.25°分辨率，37个气压层，覆盖全球
   - 是否公开: 部分公开（可通过Google Cloud获取）
...
5. **HRES预报数据**
   ...
   - 任务: 高分辨率天气预报基准对比
   - 数据规模: 0.1°分辨率，78个气压层
   - 是否公开: 部分公开（需订阅ECMWF数据服务）

### 数据处理
- 使用再分析数据作为地面真值进行对比评估
- 将不同模型的预测结果插值至统一空间分辨率（0.25°×0.25°）进行比较
- 针对三次极端天气事件设置不同的时空窗口进行局部精度和聚合精度分析
- 计算指标包括均方根误差（RMSE）、空间相关系数、时间序列对比等

### 复现难度
- ★★★☆☆（中等难度）
- 原因：三个ML模型（GraphCast、PanguWeather、FourCastNet）均已开源或提供预训练权重，但完整获取各模型的推理输出数据较为困难；ECMWF的HRES数据需要机构订阅；研究聚焦于三次特定的极端天气事件，需精确的时间窗口和气象数据；缺乏统一的代码框架和标准化的评估流程，增加了复现的复杂度。
